        ...
        <nav aria-label="Pagination" class="">
            <ul class="flex items-center flex-wrap md:flex-nowrap -space-x-px md:h-8 text-sm gap-1 justify-center">
                <li v-for="link in links">
                    <component @click="buildQuery(link.url)" :is="link.url ? Link :'span'" v-if="link.label == lt"
                        :href="link.url"
                    ...
                    <component @click="buildQuery(link.url)" :is="link.url ? Link : 'span'" v-else-if="link.label == gt"
                        :href="link.url"
                    ...
                    <component @click="buildQuery(link.url)" :is="link.url ? Link : 'span'" v-else :href="link.url"
