...
import { iStaffStatus, iStaffStatuses, iNotification } from '../../interfaces/index';
...
const props = defineProps<{
    search: string
    statuses: iStaffStatuses,
    notification: iNotification
}>()

const form = useForm<iStaffStatus>({
    id: null,
    name: ""
})
...
const detailTitle = ref('New Staff Status')

const newStaffStatus = () => {
    form.id = null
    form.name = ""
...
const editStaffStatus = (STAFF_STATUS: iStaffStatus) => {
    form.id = STAFF_STATUS.id
    form.name = STAFF_STATUS.name
    detailTitle.value = "Edit Staff Status"
    edit.value = true
}
...
watch(() => searchVal.value, debounce((value: string) => {
    ...
    router.get(route('staff-status'), data, {
        only: ['statuses', 'search'],
        preserveScroll: true,
        preserveState: true
    })
}, 500))
...
const submit = () => {
    if (edit.value) {
        form.patch(route('staff-status-update', form.id), {
            preserveScroll: true,
            preserveState: true,
            only: ['statuses', 'search', 'notification', 'errors'],
            onSuccess: () => {
                toast.add({
                    severity: 'success',
                    summary: 'Success',
        ...
        form.post(route('staff-status-store'), {
            preserveScroll: true,
            preserveState: true,
            only: ['statuses', 'search', 'notification', 'errors'],
            onSuccess: () => {
...
const deletePosition = (id: number) => {
    router.delete(route('staff-status-destroy', id), {
        preserveScroll: true,
        preserveState: true,
        only: ['statuses', 'notification'],
        onSuccess: () => {
            toast.add({
                severity: 'success',
                summary: 'Success',
...
<template>
    ...
    <AuthenticatedLayout title="Staff Statuses">
        <div class="pb-3 md:pb-8 flex gap-3 justify-between">
            ...
            <SecondaryButton @click="newStaffStatus">
                <Icon type="add" />
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="flex flex-col gap-2 md:col-span-2">
                <ListItem v-for="status in statuses.data">
                    <div>
                        <div class="text-base font-medium capitalize" v-text="status.name"></div>
                        <div class="text-xs font-light flex gap-1">
                            <span>Members</span>
                            <span v-text="status.members"></span>
                        </div>
                    </div>
                    <div class="flex flex-wrap gap-1">
                        <SecondaryButton @click="editStaffStatus(status)">
                            <Icon class="h-4 w-4" type="edit" /><span class="hidden md:block">edit</span>
                        </SecondaryButton>
                        <SecondaryButton @click="deletePosition(status.id)">
                            <Icon class="h-4 w-4" type="delete" /><span class="hidden md:block">Delete</span>
                        </SecondaryButton>
                    </div>
                </ListItem>
                <Paginator :items="statuses" />
            </div>
