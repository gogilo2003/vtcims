...
@section('content')
    <div class="row">
    	<div class="col-md-4">
    		<div class="card">
                <form id="testForm" method="post" action="{{ ($errors->count() && old('id')) ? route('admin-eschool-examinations-tests-edit-post') : route('admin-eschool-examinations-tests-add-post') }}" role="form" accept-charset="UTF-8" enctype="multipart/form-data">
    			    <div class="card-body">
                        ...
                        @else
                            <div class="form-group{!! $errors->has('examination') ? ' has-error':'' !!}">
                                ...
                                <select data-live-search="true" data-style="btn-link" class="form-control selectpicker" id="examination" name="examination" {!! ((old('examination')) ? ' value="'.old('examination').'"' : '') !!}>
                        ...
                        <div class="form-group{!! $errors->has('outof') ? ' has-error':'' !!}">
							...
							<input type="text" class="form-control" id="outof" name="outof" {!! ((old('outof')) ? ' value="'.old('outof').'"' : '') !!}>
                        ...
                        <div class="form-group{!! $errors->has('taken_on') ? ' has-error':'' !!}">
							...
							<input type="text" class="form-control datepicker" id="taken_on" name="taken_on" {!! ((old('taken_on')) ? ' value="'.old('taken_on').'"' : '') !!}>
                    ...
                    <div class="card-footer">
                        ...
                        <a id="cancelTestButton" style="display:none" href="JavaScript:" class="btn btn-round btn-danger"><span class="material-icons">cancel</span> Cancel</a>
    			    </div>
                </form>
    		</div>
    	</div>
			    			...
			    			<tbody>
			    				@foreach ((isset($tests) ? $tests : $examination->tests) as $test)
			    					<tr>
			    						...
			    						<td>
			    							<a href="JavaScript:" class="btn btn-fab btn-primary btn-sm btn-round editTestButton" data-test="{{ $test->toJson() }}"><i class="material-icons">edit</i></a>
			    						</td>
			    					</tr>
			    				@endforeach
			    			</tbody>
