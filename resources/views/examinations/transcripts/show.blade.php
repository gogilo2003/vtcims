...
@section('content')
    <a href="JavaScript:" data-toggle="modal" data-target="#studentTranscriptModal" class="btn btn-primary btn-round"><span class="material-icons">contacts</span> Transcripts</a>
    ...
    <form method="post" action="{{ route('admin-eschool-examinations-transcripts-post') }}" style="display: inline-block;">
        ...
        @csrf
        <button type="submit" class="btn btn-outline-primary rounded-pill"><span class="material-icons">cloud_download</span> Download</button>
    </form>
    @endisset
    @isset($intake)
    <form method="post" action="{{ route('admin-eschool-examinations-transcripts-post') }}" style="display: inline-block;">
...
$(document).ready(()=>{
    $('select#examination_group').selectpicker('val',{{ old('examination_group') ? old('examination_group') : (isset($examination_group) ? $examination_group->id : 'null' ) }})
