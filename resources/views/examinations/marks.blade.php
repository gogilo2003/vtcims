                        ...
                        <tr>
                            ...
                            @foreach ($examination->tests as $test)
                                @forelse ($student->results->where('test_id',$test->id) as $result)
                                    ...
                                    @endphp
                                    <td><input name="edited_marks[{{ $result->id }}]" type="number"
                                            max="{{ $test->outof }}" class="form-control text-center"
                                            value="{{ $score }}"></td>
                                @empty
                                    <td>
                                        <input name="marks[{{ $student->id }}][{{ $test->id }}]" type="number"
                                            max="{{ $test->outof }}" class="form-control text-center">
                                    </td>
                                @endforelse
                                @php
                ...
                <tr>
                    <td colspan="5" class="text-right">
                        ...
                        <a href="{{ route('admin-eschool-examinations-view', $examination->id) }}"
                            class="btn btn-danger btn-round"><span class="material-icons">cancel</span> Cancel</a>
