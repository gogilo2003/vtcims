...
@section('content')
    <a class="btn btn-primary btn-round" href="{{ route('admin-eschool-examinations-groups') }}"><span class="material-icons">view_list</span> Examination Groups</a>
    <a href="{{ route('admin-eschool-examinations-groups-add') }}" class="btn btn-primary btn-round"><span class="material-icons">add</span> Add Examination Group</a>
    <div class="card">
        <form id="examination_groupForm" method="post" action="{{ route('admin-eschool-examinations-groups-edit-post') }}" role="form" accept-charset="UTF-8" enctype="multipart/form-data">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <div class="form-group{!! $errors->has('title') ? ' has-error':'' !!}">
                            ...
                            <input type="text" class="form-control" id="title" name="title" value="{!! ((old('title')) ? old('title') : $examination_group->title) !!}">
                            ...
                            <tbody>
                                @foreach ($examination_group->intakes as $intake)
                                    ...
                                    @foreach ($students as $student)
                                        @php
                                            $ck = old('students') ? in_array($student->id,old('students')): in_array($student->id,$selected);
                                            $checked =  $ck ? 'checked' : '';
                                        @endphp
                                        <tr>
                                            <td class="text-center">
                                                <div class="form-check">
                                                    <label class="form-check-label">
                                                        <input class="form-check-input studentsCheckBox" type="checkbox" value="{{ $student->id }}" name="students[]" {{ $checked }}>
...
@section('scripts_bottom')
    <script type="text/javascript">
		$(document).ready(function(){
            ...
            $('select#intake').selectpicker('val',{!! ((old('intake')) ? json_encode(old('intake')) : ($examination_group->intakes->count() ? json_encode($examination_group->intakes->pluck('id')->toArray()) : 'null')) !!})
