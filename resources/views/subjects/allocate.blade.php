...
@section('content')
    <div class="row">
    	<div class="col-md-4">
    		<div class="card">
	    		<form id="subjectAllocationForm" method="post" action="{{route('admin-eschool-subjects-allocate-post')}}" role="form" accept-charset="UTF-8" enctype="multipart/form-data">
	    			...
	    			<div class="card-body">
	    				...
	    				<div class="form-group bmd-form-group is-filled is-focused{!! $errors->has('subject') ? ' has-error':'' !!}">
	    					...
	    					<select data-live-search="true" class="form-control selectpicker" data-style="btn btn-link" id="subject" name="subject"{!! ((old('subject')) ? ' value="'.old('subject').'"' : '') !!}>
	    				...
	    				<div class="form-group bmd-form-group is-filled is-focused{!! $errors->has('class') ? ' has-error':'' !!}">
	    					...
	    					<select data-live-search="true" class="form-control selectpicker" data-style="btn btn-link" id="class" name="class"{!! ((old('class')) ? ' value="'.old('class').'"' : '') !!}>
	    				...
	    				<div class="form-group bmd-form-group is-filled is-focused{!! $errors->has('teacher') ? ' has-error':'' !!}">
	    					...
	    					<select data-live-search="true" class="form-control selectpicker" data-style="btn btn-link" id="teacher" name="teacher"{!! ((old('teacher')) ? ' value="'.old('teacher').'"' : '') !!}>
	    			...
	    			<div class="card-footer">
		    			...
		    			<a href="JavaScript:" id="cancelSubjectAllocationButton" class="btn btn-round btn-danger" style="display: none"><i class="material-icons">cancel</i> Cancel</a>
	    			</div>
	    		</form>
	    	</div>
    	</div>
    			...
    			<tbody>
    				@foreach ($allocations as $allocation)
    					<tr>
    						...
    						<td>
    							<a href="JavaScript:" data-allocation="{{ $allocation->toJson() }}" class="btn btn-fab btn-round btn-primary btn-sm editSubjectAllocation"><i class="material-icons">edit</i></a>
    						</td>
    					</tr>
    				@endforeach
    			</tbody>
