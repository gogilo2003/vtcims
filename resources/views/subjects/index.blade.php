...
@section('content')
            ...
            <div class="card">
                <div class="card-body">
                    <form id="subjectForm" method="post"
                        action="{{ $errors->count() && old('id') ? route('admin-eschool-subjects-edit-post') : route('admin-eschool-subjects-add-post') }}"
                        ...
                        <div class="form-group bmd-form-group{!! $errors->has('subject_code') ? ' has-error' : '' !!}">
                            ...
                            <input type="text" class="form-control" id="subject_code" name="subject_code"
                                {!! old('subject_code') ? ' value="' . old('subject_code') . '"' : '' !!}>
                        ...
                        <div class="form-group bmd-form-group{!! $errors->has('subject_name') ? ' has-error' : '' !!}">
                            ...
                            <input type="text" class="form-control" id="subject_name"
                                name="subject_name"{!! old('subject_name') ? ' value="' . old('subject_name') . '"' : '' !!}>
                            {!! $errors->has('subject_name')
                        ...
                        <div class="form-group bmd-form-group is-filled is-focused{!! $errors->has('courses') ? ' has-error' : '' !!}">
                            ...
                            <select multiple="multiple" data-style="btn btn-link"
                                class="form-control bmd-form-control selectpicker" id="courses" name="courses[]">
                        ...
                        <button type="submit" class="btn btn-primary btn-round"><span class="material-icons">save</span>
                            Save</button>
                        <a href="JavaScript:" id="cancelSubjectEdit" class="btn btn-danger btn-round"
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @foreach ($subject->courses as $course)
                                                <a href="{{ route('admin-eschool-courses-view', $course->id) }}"
                                                    class="btn btn-sm btn-round btn-outline-primary"
                                                    title="{{ $course->name }}">{{ $course->code }}</a>
                                            @endforeach
                                        </td>
                                        <td>
                                            <a href="JavaScript:"
                                                class="btn btn-primary btn-fab btn-round btn-sm editSubjectButton"
                                                data-courses="{{ $subject->course_ids }}"
                                                data-subject="{{ $subject->toJson() }}"><i
                                                    class="material-icons">edit</i></a>
                                            <a href="{{ route('admin-eschool-subjects-view', $subject->id) }}"
                                                class="btn btn-sm btn-fab btn-round btn-info"><i
                                                    class="material-icons">zoom_in</i></a>
                                            <a href="JavaScript:"
                                                class="btn btn-fab btn-round btn-danger btn-sm deleteSubjectButton"
                                                data-id="{{ $subject->id }}" data-name="{{ $subject->name }}"><i
                        ...
                        <form id="deleteSubjectForm" method="post" action="{{ route('admin-eschool-subjects-delete') }}"
                            role="form" accept-charset="UTF-8" enctype="multipart/form-data">
    ...
    <!-- Modal -->
    <div class="modal fade" id="showCourseDialog" data-align="middle" tabindex="-1" role="dialog"
        aria-labelledby="ShowCourse">
