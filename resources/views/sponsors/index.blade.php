...
@section('sidebar')
    ...
    @include('eschool::sponsors.sidebar')
@endsection
...
@section('content')
    <div class="row">
    	<div class="col-md-4">
    		<div class="card">
    			<div class="card-body">
    				<form id="sponsorForm" method="post" action="{{ ($errors->count() && old('id')) ? route('admin-eschool-sponsors-edit-post') : route('admin-eschool-sponsors-add-post') }}" role="form" accept-charset="UTF-8" enctype="multipart/form-data">
    					<div class="form-group{!! $errors->has('sponsor_name') ? ' has-error':'' !!}">
    						<label class="bmd-label-floating" for="sponsor_name">Sponsor name</label>
    						<input type="text" class="form-control" id="sponsor_name" name="sponsor_name"{!! ((old('sponsor_name')) ? ' value="'.old('sponsor_name').'"' : '') !!}>
    						{!! $errors->has('sponsor_name') ? '<span class="text-danger">'.$errors->first('sponsor_name').'</span>' : '' !!}
    					</div>
                        <div class="form-group{!! $errors->has('contact_person') ? ' has-error':'' !!}">
                            <label class="bmd-label-floating" for="contact_person">Contact Person</label>
                            <input type="text" class="form-control" id="contact_person" name="contact_person" {!! ((old('contact_person')) ? ' value="'.old('contact_person').'"' : '') !!}>
                            {!! $errors->has('contact_person') ? '<span class="text-danger">'.$errors->first('contact_person').'</span>' : '' !!}
                        </div>
                        ...
                        <div class="form-group{!! $errors->has('phone_number') ? ' has-error':'' !!}">
                            ...
                            <input type="text" class="form-control" id="phone_number" name="phone_number" {!! ((old('phone_number')) ? ' value="'.old('phone_number').'"' : '') !!}>
                            {!! $errors->has('phone_number') ? '<span class="text-danger">'.$errors->first('phone_number').'</span>' : '' !!}
                        </div>
                        <div class="form-group{!! $errors->has('box_number') ? ' has-error':'' !!}">
                            <label class="bmd-label-floating" for="box_number">Box Number</label>
                            <input type="text" class="form-control" id="box_number" name="box_number" {!! ((old('box_number')) ? ' value="'.old('box_number').'"' : '') !!}>
    					...
    					<a href="JavaScript:" id="cancelSponsorEdit" class="btn btn-danger btn-round" style="display: none"><i class="material-icons">cancel</i> Cancel</a>
    				</form>
    			</div>
    		</div>
    	</div>
    						...
    						<tbody>
    							@foreach ($sponsors as $sponsor)
    								<tr>
                                        ...
                                        <td>{{ $sponsor->email }}</td>
    									<td>
    										<a href="JavaScript:" class="btn btn-primary btn-fab btn-round btn-sm editSponsorButton" data-sponsor="{{ $sponsor->toJson() }}"><i class="material-icons">edit</i></a>
    										<a href="JavaScript:" class="btn btn-fab btn-round btn-danger btn-sm deleteSponsorButton" data-id="{{ $sponsor->id }}" data-name="{{ $sponsor->name }}"><i class="material-icons">delete</i></a>
    									</td>
    								</tr>
    							@endforeach
    						</tbody>
    					</table>
    					<form id="deleteSponsorForm" method="post" action="{{route('admin-eschool-sponsors-delete')}}" role="form" accept-charset="UTF-8" enctype="multipart/form-data">
