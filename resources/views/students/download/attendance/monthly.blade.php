    ...
    <div class="table-round mb-3">
        <table class="table my-0">
            ...
            <tr>
                <td>{{ $classes }}</td>
                <td>{{ $subject->name }}</td>
                <td>{!! $month_start->format('l, j\<\s\u\p\>S\<\/\s\u\p\> F Y') !!}</td>
                <td>{!! $month_end->format('l, j\<\s\u\p\>S\<\/\s\u\p\> F Y') !!}</td>
            </tr>
        </table>
    </div>
    <div class="table-round mb-3">
        <table class="table black table-sm table-bordered table-striped my-0">
            <thead class="thead-light">
                <tr>
                    <th rowspan="2"></th>
                    <th rowspan="2" class="text-uppercase text-center">Admission No</th>
                    <th rowspan="2" class="text-uppercase text-center">Name</th>
                    @foreach ($days as $key => $week)
                        <th class="text-uppercase text-center" colspan="{{ count($week) }}">{{ $key }}</th>
                    @endforeach
                </tr>
                <tr>
