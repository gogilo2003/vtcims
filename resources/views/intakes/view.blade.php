        ...
        <tr>
            ...
            <td>
                @foreach ($intake->staff_subjects as $staff_subject)
                    <a href="{{ route('admin-eschool-subjects-view', $staff_subject->subject->id) }}"
                        class="btn btn-round btn-sm btn-outline-info">{{ $staff_subject->subject->name }}-{{ $staff_subject->staff->name }}</a>
                @endforeach
            </td>
        </tr>
    </table>
