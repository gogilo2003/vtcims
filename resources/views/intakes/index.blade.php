...
@section('content')
    <div class="row">
    	<div class="col-md-4">
    		<div class="card">
    			<div class="card-body">
    				<form id="intakeForm" method="post" action="{{($errors->count() && old('id')) ? route('admin-eschool-intakes-edit-post') : route('admin-eschool-intakes-add-post') }}" role="form" accept-charset="UTF-8" enctype="multipart/form-data">
		    			...
		    			<div class="form-group{!! $errors->has('course') ? ' has-error':'' !!}">
		    				...
		    				<select class="form-control selectpicker" data-style="btn btn-link" data-live-search="true" id="course" name="course" {!! ((old('course')) ? ' value="'.old('course').'"' : '') !!}>
		    			...
		    			<div class="form-group{!! $errors->has('instructor_incharge') ? ' has-error':'' !!}">
		    				...
		    				<select class="form-control selectpicker" data-style="btn btn-link" data-live-search="true" id="instructor_incharge" name="instructor_incharge" {!! ((old('instructor_incharge')) ? ' value="'.old('instructor_incharge').'"' : '') !!}>
		    				...
		    				{!! $errors->has('instructor_incharge') ? '<span class="text-danger">'.$errors->first('instructor_incharge').'</span>' : '' !!}
		    			</div>
		    			...
		    			<a href="JavaScript:" id="cancelEditButton"class="btn btn-round btn-danger" style="display: none;"><i class="material-icons">cancel</i> Cancel</a>
		    		</form>
    			</div>
    		</div>
    	</div>
    						...
    						<tbody>
    							@foreach ($intakes as $intake)
    								<tr>
    									...
    									<td>
    										<a href="JavaScript:" class="btn btn-primary btn-round btn-fab btn-sm editIntake" data-intake="{{ $intake->toJson() }}"><i class="material-icons">edit</i></a>
    									</td>
    								</tr>
    							@endforeach
    						</tbody>
